...
# Load data
my_results = pd.read_csv(data_directory + 'ML_results_regression.csv')
my_metadata = pd.read_csv(data_directory + 'metadata_regression.csv')
my_datasets = my_metadata['dataset']
# # # Algorithm and experiments parameters
# Random configuration selection percentage
configuration_percentage = 1
# Train-test split percentage
train_percentage = 0.9
# Set performance threshold
for thresh_perf in [0.9999, 0.999, 0.99, 0.97, 0.95]:
    ...
    for run_ in range(len(results_on_performance_update_vs_not)):
        ...
        while keep_loop == 1:
            ...
            max_performance_partition = np.zeros((len(train_datasets), len(configurations_partition))) + 0.001
        ...
        performance_ratio_train.loc[np.isnan(performance_ratio_train)] =\
            0.001 / max_performance_original.loc[np.isnan(performance_ratio_train)]
        ...
        performance_ratio_test.loc[np.isnan(performance_ratio_test)] =\
            0.001 / max_performance_original_test.loc[np.isnan(performance_ratio_test)]
    ...
    # Save results
    results_on_performance_update_vs_not.to_csv(output_directory + 'MetaTrees_regression_' + str(thresh_perf) +\
